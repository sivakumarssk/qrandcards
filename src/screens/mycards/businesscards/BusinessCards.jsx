...
// Create a circular image via canvas (for profile/logo).
...
// Helper: Group an array into rows (each row having count items)
...
function BusinessCards() {
  ...
  // Mapping for social and UPI icons.
  const socialIcons = {
    Website: websiteIcon,
  ...
  const fetchPrices = async () => {
    try {
      ...
      if (response.data) {
        const { totalpriceBusiness, dicountpriceBusiness } = response.data;
        setPrices({ totalpriceBusiness, dicountpriceBusiness });
      }
    } catch (error) {
  ...
  const handleDownloadPDF = async () => {
    ...
    // Build Contact Details using rich text arrays.
    const contactDetails = [];
    if (formData.phone) {
      contactDetails.push({
        columns: [
          { image: await convertToDataURL(whatsappImage), width: 20, height: 20 },
          {
            text: [
    ...
    if (formData.email) {
      contactDetails.push({
        columns: [
          { image: await convertToDataURL(EmailIcon), width: 20, height: 20 },
          {
            text: [
    ...
    if (formData.address) {
      contactDetails.push({
        columns: [
          { image: await convertToDataURL(AddressIcon), width: 20, height: 20 },
          {
            text: [
    ...
    const businessInfo = {
      table: {
        widths: ["*"],
        body: [
          [{ text: "Business Info", style: "sectionTitle" }],
          [{
            text:
              `${formData.gstno ? "GST No: " + formData.gstno + "\n" : ""}` +
              `${formData.bussinestimings ? "Business Timings: " + formData.bussinestimings : ""}`,
      ...
      content: [
        ...
        galleryItems.length > 0 && {
          table: {
            widths: ["*"],
            body: [
              [{ text: "Other Business Images", style: "sectionTitle" }],
    ...
    // Wrap all content in an outer container table of fixed width and center it.
    ...
    const fileName = formData.name
      ? `${formData.name.replace(/\s+/g, "_")}_BusinessCard.pdf`
      : "BusinessCard.pdf";
  ...
  const updateBusinessCount = async () => {
    try {
      ...
      console.log("Business count updated successfully!");
    } catch (error) {
      console.error("Error updating Business count:", error);
    }
  };
  ...
  const handlePDFPayment = async () => {
    ...
    try {
      const response = await axios.post("https://admin.qrandcards.com/api/create-order", {
        amount: prices?.dicountpriceBusiness || 185,
        currency: "INR",
      });
    ...
    return (
      <div className="p-6 bg-gray-100 min-h-screen mt-[5%] flex flex-col justify-center items-center">
        ...
        >
          ...
          {/* Business Info */}
          <div id="bussines-section" className="mb-6">
            <h3 className="bg-blue-500 text-center text-white py-2 px-4 rounded-t-lg w-full">
              Business Info
            </h3>
            <p className="border p-4 rounded-b-lg">
              ...
              {formData.bussinestimings && (
                <>
                  <span className="font-bold">Business Timings: </span>
                  {formData.bussinestimings}
                </>
              )}
            </p>
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen mt-[14%] lg:mt-[4%]">
      ...
      >
        ...
        <div className="mb-4">
          <label className="block mb-2">Business Timings</label>
          <input
            type="text"
